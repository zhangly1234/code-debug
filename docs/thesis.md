...
不论是基于串口还是基于网络，GDB和 main-stub 都使用 RSP 协议进行通信。RSP协议的包（packet）如下图所示，
第一个是信息头，接着是信息，在分隔符之后跟着校验值，这个协议比较简单，实现难度不高，因此，side-stub也使用RSP协议进行通信。
RSP如下图所示：

图片

二者的协议是一致的，但在具体通信上，main-stub主要以同步的消息机制为主（这是因为GDB一般是触发断点，被调试操作系统停下之后，
才开始收集信息的，GDB的调试机制是同步的），相比之下，side-stub主要依赖内核插桩机制，在插桩触发之后收集数据，eBPF程序运行完毕后操作系统
继续运行，因此大部分的信息都会以异步的方式传送给GDB。这种异步的消息处理方式提供了更高的并发性和响应性。然而，异步的消息可能会与同步的消息重合
，这就要求和 side-stub 通信的 GDB 模块能处理好异步信息，特别是在传送同步信息时
被异步信息流打断的情况。

在RSP协议有规定，同步的消息以#开头，异步的消息以%开头。利用这个特性，我们设计了一个消息处理流程,可以确保消息的有序交付和处理：GDB中
负责和side-stub通信的模块
逐字节接收来自side-stub的消息，默认情况下就按同步信息流处理，如果发现接收到了字符“%”，则接下来接收到字节流都放入异步消息处理例程，
直到接收到"#"符号并接着接收了两个校验值后，再返回原来的同步消息处理流程。这样，就算同步消息被异步消息打断，同步消息和异步消息的接收模块
都能正常运作。

由于多个 eBPF 程序不会并发运行，异步消息流不会互相重叠。运行 eBPF 程序时，操作系统其他部分都是不运行的，因此负责收发同步消息的用户态
进程也不运行，eBPF发送完了才会回到同步消息的处理流程，因此异步消息不会被同步消息打断。

图片

GDB 支持用 python 语言编写扩展，因此我们用 python 编写了和 side-stub 通信的模块。这样做的好处有两个，其一是不需要修改 GDB 的源代码，
只需要自行编译一份支持 python 扩展的GDB即可（我们使用的Ubuntu20.04的包管理器自带的GDB，和risc-v工具链[https://github.com/riscv-collab/riscv-gnu-toolchain]
提供的 GDB 可执行文件都不包含 python 扩展），其二是我们编写的这个模块运用了多线程机制，且有很多字符串处理流程，python 语言对这两个
功能支持得很好。如果用C语言编写，代码量可能要翻倍，且可读性，可维护性不佳。



实现：移植，升级，对eBPF以外的符号信息的依赖；


#### 4.2.3 VScode 同时连接被跟踪操作系统的 OpenOCD 和 eBPF 的实施方案
