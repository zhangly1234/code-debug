...
不论是基于串口还是基于网络，GDB和main-stub都使用GDB的RSP协议进行通信。RSP协议的包（packet）如下图所示，
...
才开始收集信息的，GDB的调试机制是同步的），而side-stub主要依赖内核插桩机制，在插桩触发之后收集数据，eBPF程序运行完毕后操作系统
继续运行，因此大部分的信息都会以异步的方式传送给GDB。这就要求和side-stub通信的GDB模块能处理好异步信息，特别是在传送同步信息时
被异步信息流打断的情况。

在RSP协议有规定，同步的消息以#开头，异步的消息以%开头。利用这个特性，我们设计了一个消息处理流程：GDB中负责和side-stub通信的模块
...
GDB 支持用 python 语言编写扩展，因此我们用 python 编写了和side-stub通信的模块。这样做的好处有两个，其一是不需要修改 GDB 的源代码，
