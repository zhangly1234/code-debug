...
在线调试的优势是,对本地计算机性能要求不高，无需在本地配置开发环境，便于分享协作，可以构建高效的程序
开发教学平台。

近年来，具有高度的可定制性的轻量级集成开发环境，如 Sublime Text、Atom 和 Visual Studio Code，已经迅
速普及。然而，轻量级 IDE 对操作系统在线调试的支持非常有限。鉴于轻量级集成开发环境的日益普及，以及我
们对操作系统在线调试支持的局限性的观察，我们基于 Visual Studio Code（最流行和最广泛使用的轻量级 IDE
之一）设计和实现了一个操作系统在线开发、调试环境。
...
OpenVSCode Server 是 VS Code 的一个分支，它在 VSCode 的五层架构的基础上增加了服务器层，使其可以在浏
览器中运行，并与提供远程开发环境的网络服务器通信。

在用户通过浏览器访问远程调试工具的网站之后，服务器会返回一个网页版本的 VSCode 编辑器。网页版 VSCode
是用 openvscode-server 开源软件实现的。

用户可以在网页版 VSCode 上编辑服务器上的源代码、使用服务器上的终端。我们在服务器里配置好了 Qemu 虚拟
机和 GDB、Rust 工具链。用户可以自行通过终端命令调用 Qemu、GDB 等工具手动调试，也可以通过网页版
VSCode 上的调试插件调试。

如果用户选择用调试插件进行调试，那么调试插件做的第一步是编译内核并获取可执行的镜像文件和用于调试的符
号信息文件。以下以 rCore-Tutorial-v3[5]操作系统为例，阐述如何获取内核的符号信息文件。

#### 2.2.1 编译

在使用默认编译参数的情况下，编译 rCore-Tutorial-v3 生成的操作系统镜像和调试信息文件难以用于操作系统
调试，因为编译器进行了很多优化，且删除了大部分用于调试的符号信息。因此，我们需要修改编译参数，使编译
器尽量不优化代码、尽量不删除调试信息。

一般情况下，用 cargo 创建的 rust 项目可用 release, debug 两种模式进行编译，其中 release 模式对代码进
行较强等级的优化，删除了调试相关的信息，而 debug 模式则对代码进行较弱等级的优化并保留调试相关的信息
，比较符合我们的需求。但是，rCore-Tutorial-v3 不支持使用 debug 模式进行构建。所以，为了正常进行调试
